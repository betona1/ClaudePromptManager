...
#### Case 1: Claude 응답 중 사용자 추가 입력 (DB + Sheets 모두 누락)

Claude Code가 tool을 실행하는 **도중에 사용자가 추가로 입력한 메시지**는 "새 프롬프트"가 아닌 "현재 턴의 추가 컨텍스트"로 처리됩니다. 이 경우 `UserPromptSubmit` hook 자체가 호출되지 않아 **DB에도 기록되지 않습니다.**

```
[사용자] "기능 구현해줘"          ← hook 발동 O, DB 기록 O
[Claude] tool 실행 중...
[사용자] "추가로 이것도 해줘"     ← hook 미발동 X, DB 기록 X  ⚠️
[Claude] 작업 완료
[사용자] "다음 작업 해줘"         ← hook 발동 O, DB 기록 O
```

> **이것은 Claude Code의 동작 방식에 의한 제한사항**이며, CPM에서 해결할 수 없습니다. 중요한 프롬프트는 Claude가 응답을 완료한 후 새 턴에서 입력하세요.
