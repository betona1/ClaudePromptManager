...
| Claude 작업 중 추가 프롬프트 | O (on_stop에서 복구) | O (복구 후 append) | Stop hook 시점 |
...
#### Case 1: Claude 응답 중 사용자 추가 입력 (자동 복구)

Claude Code가 tool을 실행하는 **도중에 사용자가 추가로 입력한 메시지**는 `UserPromptSubmit` hook이 호출되지 않습니다. 하지만 Claude Code는 이 메시지들을 transcript JSONL 파일에 `queue-operation` 타입으로 기록합니다.

**CPM의 `on_stop.py` hook이 이를 자동 복구합니다:**

```
[사용자] "기능 구현해줘"          ← on_prompt.py hook → DB 기록 O
[Claude] tool 실행 중...
[사용자] "추가로 이것도 해줘"     ← hook 미발동, transcript에 queue-operation 기록
[사용자] "이것도 참고해"          ← hook 미발동, transcript에 queue-operation 기록
[Claude] 작업 완료
         ↓
         on_stop.py hook 실행
         ↓
         1. 마지막 wip 프롬프트 → 응답요약 업데이트
         2. transcript 파싱 → queue-operation 메시지 발견
         3. DB에 없는 메시지 → 자동 INSERT (source='hook-queue')
```

> 중간 입력은 `source='hook-queue'`로 표시되어 일반 프롬프트(`source='hook'`)와 구분됩니다.

#### Case 2: Google Sheets만 누락 (DB는 정상)
